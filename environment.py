...
# Set to True to disable colour coding (useful if colours are not displaying correctly)
...
class Environment:
        ...
        for line in f:
            ...
            if self.n_rows is None or self.n_cols is None:
                try:
                    ...
                    self.obstacle_map = [
                        [0 for _ in range(self.n_cols)] for __ in range(self.n_rows)]
                    self.hazard_map = [
                        [0 for _ in range(self.n_cols)] for __ in range(self.n_rows)]
                except ValueError:
            ...
            #   transition model
                ...
                try:
                    ...
                    self.double_move_probs = {
                        k: v for (k, v) in zip(ROBOT_ACTIONS, probs)}
                except ValueError:
            ...
            if line[0] in ['/', '\\']:
                ...
                symbols = [s.replace('\\', '').replace(
                    '/', '').replace('_', '') for s in line.strip().split('\\__/')]
                symbols = [s for s in symbols if len(
                    s) > 0]    # remove empty symbols
                ...
                for col, sym in enumerate(symbols):
                    ...
                    if sym == OBSTACLE:
                        ...
                        widget_init_posits_list.append(
                            (row, (2 * col) + col_offset))
    ...
    def apply_dynamics(self, state, movement):
        ...
        if movement == SPIN_LEFT or movement == SPIN_RIGHT:
            ...
            forward_robot_posit = get_adjacent_cell_coords(
                state.robot_posit, forward_direction)
            if movement == FORWARD:
                ...
                new_robot_posit = get_adjacent_cell_coords(
                    state.robot_posit, move_direction)
            ...
            for i in range(self.n_widgets):
                if forward_robot_posit in widget_cells[i]:
                    ...
                    if widget_move_type == TRANSLATE:
                        ...
                        new_centre = get_adjacent_cell_coords(
                            state.widget_centres[i], move_direction)
                        ...
                        new_cells = widget_get_occupied_cells(
                            self.widget_types[i], state.widget_centres[i], new_orient)
    ...
    def render(self, state):
        ...
        for i in range(self.n_rows):
            for j in range(0, self.n_cols, 2):
                ...
                for k in range(RENDER_CELL_TOP_WIDTH):
                    ...
                    x = (j * RENDER_CELL_TOP_WIDTH) + \
                        ((j + 1) * RENDER_CELL_SIDE_WIDTH) + k
                    buffer[y][x] = '_'
                    ...
                    if j < self.n_cols - 1:
                        ...
                        x = ((j + 1) * RENDER_CELL_TOP_WIDTH) + \
                            ((j + 2) * RENDER_CELL_SIDE_WIDTH) + k
                        buffer[y][x] = '_'

                        # draw bottom half-row lower boundary '_'
                        y = ((i + 1) * RENDER_CELL_DEPTH) + \
                            RENDER_CELL_SIDE_WIDTH
                        x = ((j + 1) * RENDER_CELL_TOP_WIDTH) + \
                            ((j + 2) * RENDER_CELL_SIDE_WIDTH) + k
                        buffer[y][x] = '_'
                ...
                for k in range(RENDER_CELL_SIDE_WIDTH):
                    ...
                    x = (j * RENDER_CELL_TOP_WIDTH) + \
                        (j * RENDER_CELL_SIDE_WIDTH) + k
                    buffer[y][x] = '/'

                    # draw top half-row up-right boundary '\'
                    y = (i * RENDER_CELL_DEPTH) + RENDER_CELL_SIDE_WIDTH - k
                    x = ((j + 1) * RENDER_CELL_TOP_WIDTH) + \
                        ((j + 1) * RENDER_CELL_SIDE_WIDTH) - k + 1
                    buffer[y][x] = '\\'
                    ...
                    if j < self.n_cols - 1:
                        ...
                        y = ((i + 1) * RENDER_CELL_DEPTH) - k
                        x = ((j + 2) * RENDER_CELL_TOP_WIDTH) + \
                            ((j + 3) * RENDER_CELL_SIDE_WIDTH) - k - 1
                        buffer[y][x] = '\\'

                        # draw bottom half-row down-left boundary '\'
                        y = ((i + 1) * RENDER_CELL_DEPTH) + \
                            RENDER_CELL_SIDE_WIDTH - k
                        x = ((j + 1) * RENDER_CELL_TOP_WIDTH) + \
                            ((j + 1) * RENDER_CELL_SIDE_WIDTH) - k + 1
                        buffer[y][x] = '\\'
        ...
        for i in range(self.n_rows):
            for j in range(self.n_cols):
                if self.obstacle_map[i][j]:
                    ...
                    y = i * RENDER_CELL_DEPTH + \
                        (RENDER_CELL_SIDE_WIDTH if j % 2 == 1 else 0) + 1
                    x = (j * RENDER_CELL_TOP_WIDTH) + \
                        ((j + 1) * RENDER_CELL_SIDE_WIDTH)
        ...
        for i in range(self.n_rows):
            for j in range(self.n_cols):
                if self.hazard_map[i][j]:
                    ...
                    y = i * RENDER_CELL_DEPTH + \
                        (RENDER_CELL_SIDE_WIDTH if j %
                         2 == 1 else 0) + RENDER_CELL_SIDE_WIDTH
        ...
        for tgt in self.target_list:
            ...
            y = ti * RENDER_CELL_DEPTH + \
                (RENDER_CELL_SIDE_WIDTH if tj %
                 2 == 1 else 0) + RENDER_CELL_SIDE_WIDTH + 1
            x = (tj * RENDER_CELL_TOP_WIDTH) + ((tj + 1) *
                                                RENDER_CELL_SIDE_WIDTH) + (RENDER_CELL_TOP_WIDTH // 2)
            # buffer[y][x] = 'T'
            buffer[y][x - 1] = 't'
            buffer[y][x] = 'g'
            buffer[y][x + 1] = 't'
        ...
        for w in range(self.n_widgets):
            ...
            w_cells = widget_get_occupied_cells(
                self.widget_types[w], state.widget_centres[w], state.widget_orients[w])
            for wi, wj in w_cells:
                # draw in top half of cell, horizontally centered
                y = wi * RENDER_CELL_DEPTH + \
                    (RENDER_CELL_SIDE_WIDTH if wj %
                     2 == 1 else 0) + RENDER_CELL_SIDE_WIDTH
                x = (wj * RENDER_CELL_TOP_WIDTH) + ((wj + 1) *
                                                    RENDER_CELL_SIDE_WIDTH) + (RENDER_CELL_TOP_WIDTH // 2)
        ...
        y = ri * RENDER_CELL_DEPTH + \
            (RENDER_CELL_SIDE_WIDTH if rj %
             2 == 1 else 0) + RENDER_CELL_SIDE_WIDTH
        x = (rj * RENDER_CELL_TOP_WIDTH) + ((rj + 1) *
                                            RENDER_CELL_SIDE_WIDTH) + (RENDER_CELL_TOP_WIDTH // 2)
        ...
        for row in buffer:
            ...
            for i, char in enumerate(row):
                ...
                if char == '(':
                    # widget start
                    next_char = row[i+1]
                    w_idx = string.ascii_lowercase.index(
                        next_char.lower()) % self.n_widgets
                    if not DISABLE_COLOUR:
